...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[Suppliers] DROP CONSTRAINT [DF_Suppliers_LastEditDate]
GO
IF @@ERROR <> 0 
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[Suppliers] DROP COLUMN [LastEditDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[Suppliers] DROP CONSTRAINT [DF_Suppliers_CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[Suppliers] DROP COLUMN [CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Suppliers_Tombstone]') and TYPE = N'U') 
   DROP TABLE [dbo].[Suppliers_Tombstone];
