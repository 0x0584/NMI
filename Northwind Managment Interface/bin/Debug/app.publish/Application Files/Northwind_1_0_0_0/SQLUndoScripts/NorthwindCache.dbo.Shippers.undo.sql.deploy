...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[Shippers] DROP CONSTRAINT [DF_Shippers_LastEditDate]
GO
IF @@ERROR <> 0 
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[Shippers] DROP COLUMN [LastEditDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[Shippers] DROP CONSTRAINT [DF_Shippers_CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
ALTER TABLE [dbo].[Shippers] DROP COLUMN [CreationDate]
GO
IF @@ERROR <> 0 
     ROLLBACK TRANSACTION;
...
IF @@TRANCOUNT > 0
IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[Shippers_Tombstone]') and TYPE = N'U') 
   DROP TABLE [dbo].[Shippers_Tombstone];
